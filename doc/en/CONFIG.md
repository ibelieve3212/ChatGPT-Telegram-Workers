...
| RICH_MESSAGE_MODE         | Rich message mode.             | `true`                                     | Long messages (>4096 chars) are sent via Telegram Rich Message (Bot API 10.1+): delivered as a single unsplit message with GFM markdown rendered natively by the client. Falls back to split plain text on failure. Set `false` to restore splitting behavior. |
