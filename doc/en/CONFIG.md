...
| RICH_MESSAGE_MODE         | Rich message mode.             | `true`                                     | Long messages (>4096 chars) are sent via Telegram Rich Message (Bot API 10.1+): delivered as a single unsplit message with GFM markdown rendered natively by the client. Note: Telegram Web / outdated clients cannot render rich messages (they show an "update app" notice) — set `false` in that case. On failure, degrades stepwise: chunked rich messages → chunked HTML → plain text. Set `false` to use chunked HTML (works on all clients), degrading to plain text on failure. |
| DEBUG_MODE                | Debug mode.                    | `false`                                    | Controls flow-tracing logs (middleware chain, group @mention detection, request retries, and other `[diag]` logs). By default only error-level logs are printed. Set `true` when troubleshooting to enable full trace logging. |
...
| SESSION_IDLE_TIMEOUT | Session idle auto-reset (seconds)     | `1800`       | **Group chats only.** Start a new session after this many seconds idle, avoiding forced coupling with old context. `0` to disable. Private chats are unaffected (resuming an earlier topic after a pause is natural in 1v1). |
